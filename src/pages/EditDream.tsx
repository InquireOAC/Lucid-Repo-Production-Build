...
const EditDream = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Side-character auto-extraction happens inside handleEditDream
      // (fire-and-forget). It links any detected names to this dream.
      navigate("/journal");
    } catch (error) {
    ...
    <div className="min-h-screen starry-background animate-page-reveal">
      ...
      <div className="px-4 py-6 pb-28 space-y-6 max-w-2xl mx-auto">
        ...
        >
          ...
          {/* Side Characters — auto-extracted named people from the dream */}
          {dreamId && (
            <>
              <motion.div whileTap={{ scale: 0.98 }}>
                <button
                ...
                >
                  <motion.div
                    className="h-9 w-9 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0"
                    animate={{ rotate: charactersOpen ? 10 : 0 }}
                    transition={{ type: "spring", stiffness: 300 }}
                  >
                    <Users className="h-4 w-4 text-primary" />
                  </motion.div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground">Side Characters</p>
                    <p className="text-xs text-muted-foreground">
                      {sideCharacters.length > 0
                        ? `${sideCharacters.length} character${sideCharacters.length > 1 ? "s" : ""} linked to this dream`
                        : "People named in your dream get auto-detected when you save"}
                    </p>
                  </div>
                  <motion.div
                    animate={{ rotate: charactersOpen ? 180 : 0 }}
                    transition={{ type: "spring", stiffness: 300, damping: 20 }}
                  >
                    <ChevronDown className="h-4 w-4 text-muted-foreground" />
                  </motion.div>
                    ...
                    <div className="pt-2 space-y-3">
                      {sideCharacters.length > 0 ? (
                        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                          {sideCharacters.map((char) => (
                            <button
                              type="button"
                              key={char.id}
                              onClick={() => setCharacterDialog({ open: true, characterId: char.id, initialName: char.name || undefined })}
                              className="flex-shrink-0 flex flex-col items-center gap-1.5"
                            >
                              <div className={cn(
                                "w-16 h-16 rounded-full overflow-hidden border-2 bg-muted flex items-center justify-center",
                                char.photo_url ? "border-primary/40" : "border-dashed border-border/60",
                              )}>
                                {char.photo_url ? (
                                  <img src={char.photo_url} alt={char.name || ""} className="w-full h-full object-cover" />
                                ) : (
                              ...
                              <span className="text-[11px] text-foreground/90 max-w-[68px] truncate text-center">
                                {char.name || "Unnamed"}
                              </span>
                            </button>
                          ))}
                          ...
                          <button
                            type="button"
                            onClick={() => setCharacterDialog({ open: true, characterId: null })}
                            className="flex-shrink-0 flex flex-col items-center gap-1.5"
                          >
                            <div className="w-16 h-16 rounded-full border-2 border-dashed border-border/60 flex items-center justify-center hover:border-primary/40 transition-colors">
                        ...
                        <div className="p-4 rounded-xl border border-border/30 bg-muted/10 text-center">
                          ...
                          <p className="text-sm font-medium mb-1">No side characters yet</p>
                          <p className="text-xs text-muted-foreground mb-3">
                            Save the dream and we'll auto-detect named people. You can also add one manually.
                          </p>
                          <Button
                            onClick={() => setCharacterDialog({ open: true, characterId: null })}
                            size="sm"
                      ...
                      {isExtractingCharacters && (
                        <p className="text-[11px] text-muted-foreground text-center">Detecting characters…</p>
                      )}
                    </div>
                  </motion.div>
                )}
      ...
      <CharacterCreatorDialog
        ...
        onOpenChange={(open) => setCharacterDialog((d) => ({ ...d, open }))}
        characterId={characterDialog.characterId}
        initialName={characterDialog.initialName}
        onSaved={async (savedId) => {
          // If this was a manual-add (no existing characterId), link the new
          // character to this dream so the chip appears in the section.
          if (!characterDialog.characterId && dreamId && user?.id) {
            const { data: dreamRow } = await supabase
              ...
              .eq("user_id", user.id)
              .maybeSingle();
            const existing = (dreamRow?.dream_character_ids as string[] | null) || [];
            if (!existing.includes(savedId)) {
              await supabase
                .from("dream_entries")
                .update({ dream_character_ids: [...existing, savedId] })
                .eq("id", dreamId)
                .eq("user_id", user.id);
            }
          }
          await refreshSideCharacters();
        }}
      />

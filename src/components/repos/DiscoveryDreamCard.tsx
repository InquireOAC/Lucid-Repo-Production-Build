...
const DiscoveryDreamCard: React.FC<DiscoveryDreamCardProps> = ({
  ...
  return (
    <div
      className="flex-shrink-0 w-[160px] sm:w-[180px] md:w-full md:min-w-0 cursor-pointer stable-card group"
      onClick={() => {
    ...
    >
      {/* Cover */}
      <div className="relative aspect-[2/3] rounded-lg overflow-hidden bg-white/5 mb-2.5">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={dream.title}
            className="w-full h-full object-cover relative z-0 transition-transform duration-300 group-hover:scale-105"
            loading="lazy"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-white/10 to-white/5 relative z-0">
